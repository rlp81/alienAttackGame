...
void EnemyPixie::orbit() {
	...
	float offsetX = std::cos(rads) * speed; // X axis offset
	float offsetY = std::sin(rads) * speed; // Y axis offset
...
void EnemyPixie::shootMissile() {
	...
	if (!(currentFrame >= DEFAULT_FRAMES_TILL__NEXT_MISSILE + lastMissileFrame || lastMissileFrame == -1)) { // Check if the Enemy Pixie can fire a Missile via time from last missile launch
